...
class HuffManCoding:
    ...
    def deCompress(self, inputPath):
        ...
        with open(inputPath, "rb") as file, open(outputPath, "w") as output:
            ...
            byte = file.read(1)
            while byte:
                # byte is in the form of hexadecimal something like(b'\x02').so getting the decimal form using ord
                byte = ord(byte)  # 2
                # Converting the decimal form into binary format
                # bin(byte) gives the result 0b11111101 like this we want 11111101 so slicing it.
                # rjust is used to fill zeroes in the starting of bin like 101 -> 000000101
                bits = bin(byte)[2:].rjust(8, "0")  # 11111111
                bitString += bits
                byte = file.read(1)
            # print(bitString)             # 0000001011110001111001001111110110110000
