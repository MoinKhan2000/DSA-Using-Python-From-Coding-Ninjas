...
def nodeWithSmallestValue(root):
    if root is None:
        return float("inf")
    else:
        return min(
            root.data,
            nodeWithSmallestValue(root.left),
            nodeWithSmallestValue(root.right),
        )
...
def getMinAndMax(root):
    if root is None:
        return -1, -1
    max = nodeWithLargestValue(root)
    min = nodeWithSmallestValue(root)
    return Pair(min, max)
